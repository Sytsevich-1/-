...
namespace ЭУМК
{
    public partial class indw : Form
    {
        public indw()
        {
            InitializeComponent();
        }
        private void indw_Load_1(object sender, EventArgs e)
        {
            using (SQLiteConnection Connect = new SQLiteConnection(@"Data Source=.\eemc.db; Version=3;"))
            {
                Connect.Open();
                string sqlExpression = "SELECT * FROM ind_w";
        ...
        private void treeView1_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            using (SQLiteConnection Connect = new SQLiteConnection(@"Data Source=.\eemc.db; Version=3;"))
            {
                ...
                string sqlExpression = "SELECT Путь_к_файлу FROM  ind_w WHERE Наименование = '" + a + "'";
                SQLiteCommand command = new SQLiteCommand(sqlExpression, Connect);
                SQLiteDataReader reader = command.ExecuteReader();
                while (reader.Read())
                {
                    string filename = reader["Путь_к_файлу"].ToString();
                    System.Diagnostics.Process.Start(Directory.GetCurrentDirectory() + "\\Самостоятельная работа\\" + filename);
                }
                reader.Close();
            }
        }
