...
namespace ЭУМК
{
    public partial class additional : Form
    {
        ...
        private void additional_Load(object sender, EventArgs e)
        {
            ...
            {
                Connect.Open();
                string sqlExpression = "SELECT * FROM additional";
        ...
        private void treeView1_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            using (SQLiteConnection Connect = new SQLiteConnection(@"Data Source=.\eemc.db; Version=3;"))
            {
                ...
                string sqlExpression = "SELECT Путь_к_файлу FROM additional WHERE Наименование = '" + a + "'";
                SQLiteCommand command = new SQLiteCommand(sqlExpression, Connect);
                SQLiteDataReader reader = command.ExecuteReader();
                while (reader.Read())
                {
                    string filename = reader["Путь_к_файлу"].ToString();
                    System.Diagnostics.Process.Start(Directory.GetCurrentDirectory() + "\\Дополнительные материалы\\" + filename);
                }
                reader.Close();
            }
        }
